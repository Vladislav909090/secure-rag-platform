...
func TestSessionServiceListUserSessionsUsesPrincipal(t *testing.T) {
	...
	mock.listUserSessions = func(ctx context.Context, got *usecase.Principal, userID string) ([]*model.UserSession, string, error) {
	...
	resp, err := (&SessionServiceServerImpl{svc: mock}).ListUserSessions(authContext("token"), &pb.ListUserSessionsRequest{UserId: "u1"})
